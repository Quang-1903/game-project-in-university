...
public class Shape {
	...
	private int[][] coords; //toa do
	private PlayScreen board;	
	...
	public Shape(BufferedImage block,int [][] coords, PlayScreen board, int color){
		this.block = block;
		this.coords = coords;
		this.board = board;
		this.color = color;
	...
	public void update() {
		...
		if(!(x + deltaX + coords[0].length > 10) && !(x + deltaX < 0)) {//dieu kien cho khoi hinh khong ra khoi le phai va le trai
			for(int row = 0; row < coords.length; row++)
				for(int col = 0; col < coords[row].length; col++)
					if(coords[row][col] != 0) {
	...
	public void rotate() {
		...
		if(x  + rotateMatrix[0].length > 20 || y + rotateMatrix.length > 20)
			return; 
